...
Expectation Maximization (EM)을 활용해서 BMM을 sample들에 fitting 하게 되는데, 
 특히 latent variable을 도입하는데, 이는 mixture component k로부터 생성된 l의posterior 분포이다. 
E-step에서는  고정해놓고, Bayes rule를 이용해서 posterior 분포
  를 업데이트 하게 된다.
M-step에서는 E-step에서 추정한 를 고정 시킨 상태에서 아래와 같이  를 추정한다.
