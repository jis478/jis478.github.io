...
- 본 논문에서는 attention module과 새로운 normalization 방법론을 적용하여 end-to-end로 학습이 가능한 방법론을 제시 함
...
- 본 논문의 가장 큰 특징은 다음과 같음
   ...
   1) 새로 추가되는 aux classifier를 통해 얻어지는 attention map을 활용하여 source와 target 도메인 간 translation에 있어서 중요한 부분과 중요하지 않은 부분을 구분해 집중할 수 있는 장점.
      ...
      - generator : 이미지 생성 시 두 도메인 간의 차이에 집중 (어떤 부분에 집중해면 더욱 진짜 처럼 생성이 되는지?) <br>
      - discriminator : 진짜 이미지와 생성 이미지 간의 차이 집중 (어떤 부분에 집중하면 진짜/가짜 구분이 잘 되는지?) <br>
...
<img src="/assets/imgs/ugatit/figure1.PNG">
...
이를 통해서 domain specific attention feature map $a_s(x) = w_s * E_s(x)$  을 구할 수 있으며, 결국 generator $G_{s->t}$는 $G_t(a_s(x))$가 된다.
...
<img src="/assets/imgs/ugatit/adalin.PNG">

$u_I$: channel-wise mean for IN <br>
$u_L$: layer-wise mean for LN <br>
$\sigma_I$: channel-wise std for IN <br>
$\sigma_L$: layer-wise std for LN <br>

~~~
다양한 normalization 비교 참고자료: <https://www.tensorflow.org/addons/tutorials/layers_normalizations>
~~~
...
$ IN(x) = \gamma*((x-\mu(x))/\sigma(x)) + \beta $ <br>
$\mu_{nc}(x) = 1/HW * \sum^H_{h=1} \sum^W_{w=1} * x_{nchw} $ <br> <br> <br>
...
- Generator의 Aux classifier $\eta_s(x)$ : Generator의 encoder가 만들어내는 각각의 feature map에 $\eta_s(x)$의 가중치 (attention)을 곱해서 encoding 작업이 더욱 효과적으로 수행 (어떤 feature map이 Generator에 있어서 더 중요한지 판단) 되도록 도와준다.  $\eta_s(x)$ 결과 값은 이미지 $x$가 domain $X_s$에서 올 확률을 의미한다.


- Discriminator의 Aux classifier $ \eta_{D_t} $: 위와 유사하게 $ \eta_{D_t} $의 파라미터는 Discriminator의 feature map 중 어떤 것이 중요한지에 대한 가중치 값임. 즉,  결과 값은 input 이미지 $x$가 진짜 이미지 ($X_t$) 에서 올 확률을 의미한다.
...
Discriminator CAM loss에서는 $\eta_{D_t} $는 도메인이 아닌 진짜/가짜에 대한 구분을 하는 loss 반영을 위해 LSGAN loss 형태로 구성을 한다.
  ...
  $L_{cam}^{D_t} = E_{x~X_t}\[\eta_{D_t}(x))^2\] + E_{x~X_s}\[(1-\eta_{D_t}(G_{s->t}(x))^2\]$
...
<img src="/assets/imgs/ugatit/loss.PNG">
...
<img src="/assets/imgs/ugatit/figure2.PNG">
...
<img src="/assets/imgs/ugatit/figure3.PNG">
<img src="/assets/imgs/ugatit/table1.PNG">


#### 3.3.3 Qualitative Evaluation


<img src="/assets/imgs/ugatit/figure4.PNG">
<img src="/assets/imgs/ugatit/table2.PNG">
...
<img src="/assets/imgs/ugatit/figure5.PNG">

u-gat-it이 전반적으로 가장 좋은 스코어를 보이며, 유사하게 attention module를 사용한 AGGAN보다 더욱 좋은 성능을 보이나, AGGAN 논문에서는 반대로 AGGAN 성능이 높다고 주장.
기존 anime translation에 특화된 CartoonGAN의 경우 주요 feature 중에 하나인 "눈" translation에 실패하면서 KID 점수가 저하됨.
(CartoonGAN의 경우, shape translation 보다는 style transfer와 유사하게 texture를 변형하는 쪽에 중점을 두고 있기 때문에 눈 모양에 변화가 거의 없음)
